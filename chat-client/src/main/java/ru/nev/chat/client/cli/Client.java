...
public class Client {
  ...
  private static void start(String host, int port) throws IOException, InterruptedException {
    ChatResponseHandler handler = message -> System.out.printf("@%s: %s%n", message.getUser().getName(), message.toChat());
    ChatClient chatClient = new ChatClient(host, port, MessageConverterFactory.INSTANCE.make(), handler);
    chatClient.start();
